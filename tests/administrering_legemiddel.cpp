...
TEST_CASE("Create AdministreringLegemidel from node",
  ...
  REQUIRE(administrasjonsvei[0].value() == "53");

  auto kanknuses = administrering.kanknuses();
  REQUIRE(kanknuses.has_value() == false);

  auto kanapnes = administrering.kanapnes();
  REQUIRE(kanapnes.has_value() == false);
  ...
  auto deling = administrering.deling();
  REQUIRE(deling.has_value() == false);

  auto enhetdosinerg = administrering.enhetdosering();
  REQUIRE(enhetdosinerg.has_value() == true);
  REQUIRE(enhetdosinerg.value()[0].value() == "13");

  auto kortdose = administrering.kortdose();
  REQUIRE(kortdose.has_value() == false);

  auto forhandsregelinntak = administrering.forhandsregelinntak();
  REQUIRE(forhandsregelinntak.has_value() == false);
}
