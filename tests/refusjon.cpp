...
TEST_CASE("Test iteratoring of a node", "[refusjon]")
{
  ...
  festlib::xml::Refusjon refusjon = festlib::test_get_refusjon(node);

  auto refrefusjonsgruppe = refusjon.refrefusjonsgruppe();

  REQUIRE(refrefusjonsgruppe.empty() == false);
  REQUIRE(refrefusjonsgruppe[0].compare("ID_18258D51-519E-47AC-9430-D284BA1A1514") == 0);

  auto gyldigfradato = refusjon.gyldigfradato();
  REQUIRE(gyldigfradato.compare("2023-01-01") == 0);
}
