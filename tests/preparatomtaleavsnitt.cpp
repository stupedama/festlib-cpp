...
TEST_CASE("Get the value of a preparatomtaleavsnitt", "[Preparatomtaleavsnitt]")
{
  ...
  auto avsnitt = festlib::test_get_preparatomtaleavsnitt(node);
  REQUIRE(avsnitt.lenke().empty() == false);
  REQUIRE(avsnitt.lenke().compare("https://www.testing.com/lenke/is") == 0);
  
  auto avsnittoverskrift = avsnitt.avsnittoverskrift();
  REQUIRE(avsnittoverskrift.value().compare("SCHUMANN_001") == 0);
}
