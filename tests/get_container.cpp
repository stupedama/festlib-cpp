...
const char xml_string[] =
    ...
    "S='2.16.578.1.12.4.1.1.7477' DN='Sublingval bruk' /><Administrasjonsvei "
    "V='53' S='2.16.578.1.12.4.1.1.7477' DN='Oral bruk' /><EnhetDosering "
    "V='13' S='2.16.578.1.12.4.1.1.7480' DN='tablett' "
    "/></"
...
TEST_CASE("Test iteratoring of a node", "[get_container]") {
  ...
  node = node.child("KatLegemiddelMerkevare")
             ...
             .child("AdministreringLegemiddel");

  auto res = festlib::xml::get_container<festlib::xml::Cv>(
      node, "Administrasjonsvei",
      [](const pugi::xml_node &n) { return festlib::xml::get_cv(n); });

  REQUIRE(res[0] == "53");
  REQUIRE(res[1] == "53");
  REQUIRE(res.size() == 2);

  auto new_res = festlib::xml::get_container<festlib::xml::Cv>(
      node, "EnhetDosering",
      [](const pugi::xml_node &n) { return festlib::xml::get_cv(n); });

  REQUIRE(new_res.size() == 1);
  REQUIRE(new_res[0] == "13");
}
