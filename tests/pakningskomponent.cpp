...
TEST_CASE("Create class Pakningskomponent from node", "[Pakningskomponent]") {
  ...
  auto pakningskomponent = festlib::xml::get_pakningskomponent(node);
  REQUIRE(pakningskomponent.empty() == true);
  //  REQUIRE(pakningskomponent[0].pakningstype().long_value().compare(
  //              "Blisterpakning") == 0);
}
